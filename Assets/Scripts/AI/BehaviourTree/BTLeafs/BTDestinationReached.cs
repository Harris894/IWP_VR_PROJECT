...
public class BTDestinationReached : BTNode
{
    ...
    public override BTResult Execute()
    {
        ...
        //if ((agentPosition - destinationPosition).sqrMagnitude < context.navAgent.height/2)
        if(context.navAgent.remainingDistance<0.1)
        {
            destination.OnDestinationReached();
            context.animatorController.SetBool("walk", false);
